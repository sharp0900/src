...
public class Asteroid extends MoveableGameObject implements ISelectable, IDrawable, ICollider {
	...
	public String toString() {
		String text = ("Asteroid: " + " Location: " + this.getLocation() + " Color: [" + ColorUtil.red(getColor())
				+ ", " + ColorUtil.green(getColor()) + ", " + ColorUtil.blue(getColor()) + "]" + " Speed: "
				+ this.getSpeed() + " Direction: " + this.getHeading());
		return text;
	}
	...
	public boolean contains(Point pPtrRelPrnt, Point pCmpRelPrnt) {
		...
		if ((ptrX <= (locX + this.size)) && (ptrX >= locX) && (ptrY <= (locY + this.size)) && (ptrY >= locY)) {
			temp = true;
			this.color = (ColorUtil.argb(100, 100, 100, 255));
		} else {
			this.color = (ColorUtil.argb(100, 200, 50, 100));
		}
	...
	public void draw(Graphics g, Point pCmpRelPrnt) {
		if (this.isSelected()) {
			g.setColor(ColorUtil.argb(100,100, 100, 255));
		} else {
